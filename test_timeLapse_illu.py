...
def build_model(input_height, input_width):
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
        ...
        new_shading, rendering_mask = lambSH_layer.lambSH_layer(
            tf.ones_like(albedo), nm_pred, new_lighting_var,
            tf.ones_like(shadow), 1.)
        ...
        shadow_gen = sdNet.shadow_generator(init_sd(nm_pred, new_lighting_var),
                                            options,
                                            name='sd_generator')
        ...
        init_sky = tf.zeros((tf.shape(relit_rendering)[0], FLAGS.z_dim * 4),
                            dtype=tf.float32)
...
rendering, input_var, mask_var, new_lighting_var, train_flag = build_model(
    new_img_h, new_img_w)
...
for i in range(0, len(illu)):
    ...
    [rendering_val] = sess.run(
        [rendering],
        feed_dict={
            train_flag: False,
            input_var: img,
            mask_var: img_mask,
            new_lighting_var: new_lighting
        })
