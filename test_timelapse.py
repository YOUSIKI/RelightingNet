...
def build_model(input_height, input_width):
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
        input1_var = tf.placeholder(tf.float32,
                                    (None, input_height, input_width, 3))
        input2_var = tf.placeholder(tf.float32,
                                    (None, input_height, input_width, 3))
        ...
        albedo_, shadow_, nm_pred_, new_lighting = irnLayer(
            input1_noSky, mask_var)
        albedo, shadow, nm_pred, lighting = irnLayer(input2_noSky, mask_var)

        shading, rendering_mask = lambSH_layer.lambSH_layer(
            tf.ones_like(albedo), nm_pred, lighting, tf.ones_like(shadow), 1.)

        new_shading, rendering_mask = lambSH_layer.lambSH_layer(
            tf.ones_like(albedo), nm_pred, new_lighting, tf.ones_like(shadow),
            1.)
        ...
        shadow_gen = sdNet.shadow_generator(init_sd(nm_pred, new_lighting),
                                            options,
                                            name='sd_generator')

        g_input = tf.concat(
            [albedo, nm_pred, new_shading, residual, shadow_gen, 1 - mask_var],
            axis=-1)

        relit_rendering = renderingNet.rendering_Net(inputs=g_input,
                                                     masks=mask_var,
        ...
        init_sky = tf.random_normal(
            (tf.shape(relit_rendering)[0], FLAGS.z_dim * 4), dtype=tf.float32)
        cinput_sky1 = tf.image.resize_images(
            (relit_rendering * 2. - 1.) * mask_var, (200, 200))
        ...
        return rendering, albedo, shadow, nm_pred, lighting, new_shading, input1_var, input2_var, mask_var, train_flag
...
rendering, albedo, shadow, nm_pred, lighting, new_shading, input1_var, input2_var, mask_var, train_flag = build_model(
    new_img_h, new_img_w)
...
[
    rendering_val, albedo_val, shadow_val, nm_pred_val, lighting_val,
    shading_val
] = sess.run([rendering, albedo, shadow, nm_pred, lighting, new_shading],
             feed_dict={
                 train_flag: False,
                 input1_var: img1,
                 input2_var: img2,
...
albedo_val = np.uint8(albedo_val[0] * 255)
shadow_val = np.uint8(shadow_val[0] * 255)
normal_val = np.uint8((nm_pred_val[0] + 1) * 128)
shading_val = np.uint8(shading_val[0] * 255)

io.imsave(os.path.join(dst_dir, "rendering.png"), rendering_val)
io.imsave(os.path.join(dst_dir, "albedo.png"), albedo_val)
io.imsave(os.path.join(dst_dir, "shadow.png"), shadow_val)
io.imsave(os.path.join(dst_dir, "normal.png"), normal_val)
io.imsave(os.path.join(dst_dir, "shading.png"), shading_val)
